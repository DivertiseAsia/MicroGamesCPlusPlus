...
bool MainMenu::init()
{
	...
	//float startY = center.height - offsetY - 170;
	float menuScale = 0;
	...
	for (int i = 0; i < GameList::instance()->numberOfAvailableGames(); i++) {
		...
		//auto xPosition = 0;// i  * visibleSize.width* MMS_MENU_RADIUS_CONSTANT;//cos(rad) * visibleSize.width * MMS_MENU_RADIUS_CONSTANT;
		//auto yPosition = j*visibleSize.width *.35f;//sin(rad) * visibleSize.width * MMS_MENU_RADIUS_CONSTANT;
		...
		//if (i % 2 == 0) xPosition = visibleSize.width *.7f;
		//else xPosition = visibleSize.width - visibleSize.width *.7f	;
		log("x = %.4f : y = %.4f", xPosition, yPosition);
		//button->setPosition(Vec2(origin.x + xPosition, startY + yPosition));
		//button->setScale(0.5);
		button->setPosition(Vec2(origin.x + visibleSize.width / 2 + xPosition, startY + yPosition));
		rad += (3.141 * 2) / 6;
		if (menuScale == 0) {
			menuScale = button->getContentSize().width / visibleSize.width;
		}
		button->setScale((MMS_MENU_WIDTH_PERCENT / menuScale)-0.175);
